...
execute unless entity @s[tag=blue.tr.mats.powder] positioned ^ ^0.5 ^1.5 as @e[type=item,distance=..16,tag=!blue.tr.shaper_checked,nbt={OnGround:1b,Item:{id:"minecraft:tnt",Count:1b}}] at @s run function blue:tr/effects/shaper/place
...
execute if entity @s[tag=blue.tr.shaper.abs] if score #.temp blue.misc matches ..1 run function blue:tr/effects/shaper/emp/abs
