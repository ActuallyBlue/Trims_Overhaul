...
tellraw @s {"translate":"blue.tr.logs.unlock","fallback":"%s unlocked %s objective %s minutes ago","color":"gray","with":[{"storage":"blue:trims","nbt":"logs.temp[-1].user","color":"white","interpret":true},{"storage":"blue:trims","nbt":"logs.temp[-1].trim","extra":["'s"],"interpret":true},{"storage":"blue:trims","nbt":"logs.time","color":"white","plain":true}]}
