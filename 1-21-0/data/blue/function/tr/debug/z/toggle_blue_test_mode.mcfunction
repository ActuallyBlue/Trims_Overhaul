...
execute as ActuallyBlue if predicate blue:shared/is_sneaking at @s positioned ^ ^ ^24 run data modify storage blue:trims temp.debug_mode set from entity @n[tag=!blue.debug.display] Tags
execute as ActuallyBlue unless predicate blue:shared/is_sneaking at @s positioned ^ ^ ^6 run data modify storage blue:trims temp.debug_mode set from entity @n[tag=!blue.debug.display] Tags
