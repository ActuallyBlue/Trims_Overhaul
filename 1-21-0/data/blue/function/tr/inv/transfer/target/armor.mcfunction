function blue:tr/inv/transfer/target/store
execute unless score #.temp0 blue.misc matches 4 run return fail
...
$tellraw @s [{"text":"- ","color":"dark_gray"},{"translate":"blue.tr.given_ownership","fallback":"%s gave you ownership over %s ","color":"gray","with":[{"selector":"@a[tag=blue.tr.transfer_user,limit=1]","color":"white"},{"translate":"$(translate)","color":"$(color)"}]},{"text":"ℹ","hoverEvent":{"action":"show_text","value":{"translate":"blue.tr.info_shortcut","fallback":"Click here for a shortcut to the info menu","color":"gray"}},"clickEvent":{"action":"run_command","value":"/trigger Trims_Menu.BLUE set 50"},"click_event":{"action":"run_command","command":"/trigger Trims_Menu.BLUE set 50"},"color":"gray"}]