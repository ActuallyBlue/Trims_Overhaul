...
data modify storage blue:data trims.logs.append.trim set value {"translate":"Flow","color":"#85B2C5"}
...
execute if score #.tr.objective_type blue.config matches 1 run tellraw @s [{"storage":"blue:data","nbt":"trims.unlock_text","interpret":true,"italic":true},{"translate":"blue.tr.flow_quest","fallback":"Seven strikes aloft, then one that shakes the world","color":"gray"}]
execute if score #.tr.objective_type blue.config matches 2 run tellraw @s [{"storage":"blue:data","nbt":"trims.unlock_text","interpret":true,"italic":true},{"translate":"blue.tr.flow_task","fallback":"Omen Opener","color":"gray"}]