execute if score #.tr.tide_toggle blue.config matches 2 run return run tellraw @s {"translate":"blue.tr.empowerment_disabled","fallback":"- %s empowerment is disabled on this server.","color":"gray","with":[{"translate":"Tide","extra":["'s"],"color":"#199ADD"}]}
