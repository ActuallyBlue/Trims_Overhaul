execute if score #.tr.shaper_toggle blue.config matches 2 run return run tellraw @s {"translate":"blue.tr.empowerment_disabled","fallback":"- %s empowerment is disabled on this server.","color":"gray","with":[{"translate":"Shaper","extra":["'s"],"color":"#8F8F8F"}]}
