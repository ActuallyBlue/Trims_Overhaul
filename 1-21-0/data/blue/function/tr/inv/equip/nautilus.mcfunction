execute if score #.tr.wild_toggle blue.config matches 2 run return run tellraw @s {"translate":"blue.tr.empowerment_disabled","fallback":"- %s empowerment is disabled on this server.","color":"gray","with":[{"translate":"Wild","extra":["'s"],"color":"#00D529"}]}
