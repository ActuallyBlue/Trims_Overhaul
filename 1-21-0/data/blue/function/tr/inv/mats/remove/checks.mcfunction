execute unless data storage blue:trims inventory[].components."minecraft:trim"{material:"minecraft:dragon_egg"} unless data storage blue:trims inventory[].components."minecraft:trim"{pattern:"minecraft:dragon_egg"} run tag @s[tag=blue.tr.dragon_egg] remove blue.tr.dragon_egg
