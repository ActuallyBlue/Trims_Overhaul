...
execute if score #.tr.wayfinder_completed blue.config matches 1.. unless entity @s[tag=blue.tr.wayfinder] run return fail
...
scoreboard players add @s[advancements={blue:tr/quests/wayfinder={trial_chamber=true}}] blue.tr.structures 1
execute unless score #.temp blue.misc < @s blue.tr.structures run return fail
