...
execute if entity @s[advancements={blue:tr/quests/sentry={start_0=false,start_1=false}}] run scoreboard players add #.temp blue.misc 3
scoreboard players operation #.temp blue.misc *= #.tr.sentry_quest blue.config
execute if entity @s[advancements={blue:tr/quests/sentry={end=false}}] run summon area_effect_cloud ~ ~3 ~ {Rotation:[0,180],Radius:0,Duration:20000,Tags:["blue.tr.quest.sentry","smithed.entity","smithed.strict"],custom_particle:{type:"minecraft:block",block_state:"minecraft:air"},Particle:{type:"minecraft:block",block_state:"minecraft:air"}}
execute positioned ~ ~3 ~ store result entity @n[tag=blue.tr.quest.sentry,distance=..1,type=area_effect_cloud] Duration int 120 run scoreboard players get #.temp blue.misc
...
execute if entity @e[tag=blue.tr.quest.sentry,distance=..100,limit=1,type=area_effect_cloud] run function blue:tr/completion/sentry