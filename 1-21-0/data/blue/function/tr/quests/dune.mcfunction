...
execute at @e[distance=..12,type=falling_block,nbt={BlockState:{Name:"minecraft:suspicious_sand"}}] positioned ~-1 ~-2.5 ~-1 run tag @e[type=!#blue:shared/peaceful,dx=1,dy=4,dz=1] add blue.tr.quest.dune
execute store result score #.temp blue.misc run tag @e[tag=blue.tr.quest.dune,distance=..25] remove blue.tr.quest.dune
