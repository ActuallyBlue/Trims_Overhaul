tag @s add blue.tr.flow
advancement grant @s only blue:tr/trust flow
advancement grant @a only blue:tr/display/flow
scoreboard players set @s blue.tr.ominous_vault 0
scoreboard players set #.tr.flow_completed blue.config 1
advancement revoke @s only blue:tr/quests/flow_kills
data modify storage blue:trims logs.append.trim set value {"translate":"Flow","color":"#85B2C5"}
function blue:tr/completion/z/setup
data modify storage blue:trims flow.owner.text set from storage blue:shared player_name
data modify storage blue:trims flow.owner.color set value "#85B2C5"
execute if score #.tr.armor_ownership blue.config matches 1 run return fail
execute if score #.tr.objective_announcements blue.config matches 1 run tellraw @a [{"text":"- ","color":"dark_gray"},{"selector":"@s","color":"#85B2C5"}," ",{"translate":"blue.tr.announce_completion","fallback":"has completed the objective for","color":"gray"}," ",{"translate":"Flow","color":"#85B2C5"}]
function blue:tr/delayed/cmd/msg/flow
scoreboard players set #.template_loot blue.misc 4
loot spawn ~ ~0.5 ~ loot blue:tr/flow
scoreboard players set #.template_loot blue.misc 1
summon wind_charge ~ ~ ~2 {acceleration_power:0.1d,Motion:[0.0d,-0.5d,0.0d]}
summon wind_charge ~1.41 ~ ~1.41 {acceleration_power:0.1d,Motion:[0.0d,-0.5d,0.0d]}
summon wind_charge ~2 ~ ~ {acceleration_power:0.1d,Motion:[0.0d,-0.5d,0.0d]}
summon wind_charge ~1.41 ~ ~-1.41 {acceleration_power:0.1d,Motion:[0.0d,-0.5d,0.0d]}
summon wind_charge ~ ~ ~-2 {acceleration_power:0.1d,Motion:[0.0d,-0.5d,0.0d]}
summon wind_charge ~-1.41 ~ ~-1.41 {acceleration_power:0.1d,Motion:[0.0d,-0.5d,0.0d]}
summon wind_charge ~-2 ~ ~ {acceleration_power:0.1d,Motion:[0.0d,-0.5d,0.0d]}
summon wind_charge ~-1.41 ~ ~1.41 {acceleration_power:0.1d,Motion:[0.0d,-0.5d,0.0d]}
playsound block.trial_spawner.ominous_activate player @a[x=0] ~ ~ ~ 3 0.5
particle trial_spawner_detection_ominous ~ ~-0.5 ~ 3 3 3 0 1000