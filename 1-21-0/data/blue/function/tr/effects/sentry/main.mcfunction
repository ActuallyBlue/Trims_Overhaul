execute if score #.60 blue.misc matches 25 if entity @e[distance=..100,limit=1,type=villager] if entity @e[type=#raiders,distance=..80,limit=1] run function blue:tr/effects/sentry/defender
execute if score #.60 blue.misc matches 50 if entity @e[distance=..100,limit=1,type=villager] if entity @e[type=#raiders,distance=..80,limit=1] run particle enchanted_hit ~ ~0.9 ~ 0.3 0.3 0.3 0 5 normal @a[scores={blue.particles=2..}]
...
execute as b163102f-0-10-0-1 run tp @s[tag=blue.tr.sentry.refresh_cloud] ~ ~0.5 ~
...
execute if predicate {"condition":"entity_properties","entity":"this","predicate":{"effects":{"hero_of_the_village":{"duration":{"max":1}}}}} run function blue:tr/effects/sentry/refresh
