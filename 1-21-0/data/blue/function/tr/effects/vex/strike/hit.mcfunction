execute positioned ^ ^ ^3 as @e[type=!#blue:shared/peaceful,sort=nearest,distance=..4] unless entity b163102f-0-8-0-1 at @s on attacker if score @s blue.tr.current_trim matches 13 run summon area_effect_cloud ~ ~ ~ {Rotation:[0,180],Radius:0,Duration:1,UUID:[I;-1318907857,8,0,1],custom_particle:{type:"minecraft:block",block_state:"minecraft:air"},Particle:{type:"minecraft:block",block_state:"minecraft:air"}}
...
execute unless score #.vex.isolated blue.misc matches 1 run return fail
...
function blue:tr/effects/flash {args:" ~ ~0.5 ~"}