...
execute as @e[tag=blue.tr.coast.projectile,type=item_display] at @s run function blue:tr/effects/coast/fire/proj/main
execute if score #.20 blue.misc matches 6 if predicate blue:shared/in_shipwreck run effect give @s luck 2 1 true
execute if entity @s[tag=blue.tr.coast.propel] as @e[tag=!blue.tr.propel_check,tag=!blue.tr.coast.checked,distance=..4,type=trident] at @s run function blue:tr/effects/coast/propel