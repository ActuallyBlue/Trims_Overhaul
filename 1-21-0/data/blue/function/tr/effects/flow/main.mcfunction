...
execute as @e[type=breeze_wind_charge,tag=blue.tr.flow_proj] store result entity @s Air byte 1 run scoreboard players get #.20 blue.misc
...
execute if score #.flow.chamber blue.misc matches 1 run return run attribute @s generic.gravity modifier add blue:tr.trim -0.04 add_value
