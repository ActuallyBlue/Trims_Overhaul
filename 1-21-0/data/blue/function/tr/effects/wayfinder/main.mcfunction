...
effect give @s[predicate=blue:tr/ground_sneak] slow_falling 1 0 true
execute if predicate {"condition":"entity_properties","entity":"this","predicate":{"effects":{"minecraft:speed":{"amplifier":{"min":2}}}}} run function blue:tr/effects/wayfinder/fast
...
execute unless entity @s[tag=blue.tr.mats.star] run return fail
  ...
  execute unless score #.wayfinder.warp_cooldown blue.misc matches 1 if entity @s[scores={blue.tr.combat=0..},predicate=blue:tr/hold_compass,predicate=!blue:tr/moving] run function blue:tr/effects/wayfinder/emp/compass
