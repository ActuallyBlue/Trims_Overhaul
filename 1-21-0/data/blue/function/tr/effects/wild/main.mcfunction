...
damage @e[type=!#blue:tr/peaceful,distance=..0.6,tag=!blue.tr.wild,limit=1] 4.5 blue:tr/poke by @s
execute store success score #.wild.ambient blue.misc unless score #.wild.ambient blue.misc matches 1
execute if score #.wild.ambient blue.misc matches 0 unless block ~ ~-0.5 ~ #replaceable run particle falling_spore_blossom ~ ~0.25 ~ 0.22 0.1 0.22 1 1 normal @a[scores={blue.particles=1..}]
execute if score #.wild.ambient blue.misc matches 1 unless block ~ ~-0.5 ~ #replaceable run particle falling_spore_blossom ~ ~0.25 ~ 0.22 0.1 0.22 1 1 normal @a[scores={blue.particles=2..}]
execute store result score #.temp blue.misc if entity @e[type=marker,tag=blue.tr.wild.trap]
execute if score #.temp blue.misc > #.wild.trap_limit blue.config run kill @e[type=marker,tag=blue.tr.wild.trap,sort=furthest,limit=1]
...
execute unless predicate blue:shared/is_sneaking run return fail
effect give @e[distance=0.01..6,type=!#blue:tr/peaceful,type=!player,predicate=blue:r/5c] poison 3 2
