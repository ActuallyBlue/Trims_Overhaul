...
execute unless predicate {"condition":"entity_properties","entity":"this","predicate":{"location":{"biomes":"#blue:sandy"}}} run return fail
execute store success score #.temp blue.misc if predicate {"condition":"location_check","predicate":{"structures":"desert_pyramid"}}
execute if score #.temp blue.misc matches 0 run attribute @s generic.attack_damage modifier add blue:tr.trim 1.25 add_value
execute if score #.temp blue.misc matches 0 run return run effect give @s fire_resistance 2 0 true
attribute @s generic.attack_damage modifier add blue:tr.trim 2.5 add_value
