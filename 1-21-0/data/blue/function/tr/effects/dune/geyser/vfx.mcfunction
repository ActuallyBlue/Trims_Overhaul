particle poof ~ ~-0.8 ~
execute unless score #.20 blue.misc matches 2..10 unless score #.20 blue.misc matches 12.. run playsound block.sand.break player @a ~ ~ ~ 0.5 0.4
execute unless block ~ ~ ~ #blue:shared/passable run return run function blue:tr/effects/dune/geyser/quiet_vfx
execute unless score #.20 blue.misc matches 2..10 unless score #.20 blue.misc matches 12.. positioned ~-1.5 ~ ~-1.5 as @a[advancements={blue:tr/trust={dune=false}},dx=2,dy=4,dz=2] run damage @s 8 blue:tr/fire by @a[tag=blue.tr.dune,limit=1]
...
execute positioned ~-1.5 ~ ~-1.5 run effect give @a[advancements={blue:tr/trust={dune=true}},dx=2,dy=4,dz=2,predicate=!blue:shared/has_regen] regeneration 5 1 true
