...
execute unless score #.temp blue.misc matches -5521 run return run tag @s add blue.tr.snout.checked
execute store result score #.temp blue.misc if entity @e[tag=blue.tr.gamble.item,type=item]
execute if score #.temp blue.misc >= #.snout.gamble_limit blue.config unless score #.snout.gamble_limit blue.config matches -1 run return run tag @s add blue.tr.snout.checked
