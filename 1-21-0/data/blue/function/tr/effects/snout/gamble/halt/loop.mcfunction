...
execute at @e[type=area_effect_cloud,tag=blue.tr.mob_freeze] as @n[type=!#blue:shared/no_effects,type=!player,tag=!smithed.strict,distance=..1] run function blue:tr/effects/snout/gamble/halt/mob
...
execute if score #.clock_tick blue.misc matches 1 at @e[type=area_effect_cloud,tag=blue.tr.snout_center] run playsound item.lodestone_compass.lock player @a[distance=..32] ~ -128 ~ 0 0.75 1
execute if score #.clock_tick blue.misc matches 0 at @e[type=area_effect_cloud,tag=blue.tr.snout_center] run playsound item.lodestone_compass.lock player @a[distance=..32] ~ -128 ~ 0 1.25 1
