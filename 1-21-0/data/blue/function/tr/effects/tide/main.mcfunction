...
execute if block ~ ~ ~ water run function blue:tr/effects/tide/in_water
execute if predicate {"condition":"weather_check","raining":true} unless biome ~ ~ ~ #blue:no_rain positioned over ocean_floor at @s[dy=300] run function blue:tr/effects/tide/in_rain
execute if score #.tide.in_rain blue.misc matches 1 unless predicate {"condition":"weather_check","raining":true} run function blue:tr/effects/tide/speed
...
execute as @e[tag=!blue.tr.tide.checked,type=trident,sort=nearest] at @s run function blue:tr/effects/tide/emp/trident
