...
execute as @a[distance=..5,advancements={blue:tr/trust={ward=true}},gamemode=!spectator] positioned ~-5 ~-1.5 ~-5 run tag @s[dx=27,dy=4,dz=27] add blue.tr.ward.in_field
effect give @a[tag=blue.tr.ward.in_field,predicate=!blue:shared/has_regen] regeneration 3 0 true
effect give @a[tag=blue.tr.ward.in_field,scores={blue.food=..8}] saturation 1 0 true
execute at @a[tag=blue.tr.ward.in_field] run particle electric_spark ~ ~0.1 ~ 0.2 0 0.2 0.1 1
