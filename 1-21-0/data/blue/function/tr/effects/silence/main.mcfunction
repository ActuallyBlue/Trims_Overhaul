...
execute if score #.60 blue.misc matches 30 store success score #.remove_Silent blue.misc as @e[type=!#blue:tr/no_effects,type=!warden,type=!player,tag=!blue.tr.silence.Silent,tag=!smithed.strict,distance=0.01..16] at @s run function blue:tr/effects/silence/entity
...
execute as @e[type=item,tag=!blue.tr.checked,distance=..6,nbt={Item:{id:"minecraft:echo_shard"}}] run function blue:tr/effects/silence/emp/vortex/throw
...
execute if score #.20 blue.misc matches 7 run playsound entity.warden.heartbeat master @a[tag=blue.tr.trim.ward,tag=blue.tr.ward] ~ ~ ~ 1.8 0.9
