...
execute if score #.shaper.throw_cooldown blue.config matches -1 on origin run return run title @s actionbar {"translate":"blue.tr.shaper_throw_disabled","fallback":"Shaper's TNT throwing is disabled on this server."}
execute if score #.shaper.throw_cooldown blue.misc matches 1.. run data modify entity @s PickupDelay set value 0s
execute if score #.shaper.throw_cooldown blue.misc matches 1.. on origin run return run title @s actionbar [{"translate":"blue.tr.controlled_demolition_name","fallback":"Controlled Demolition","color":"#8F8F8F"},{"translate":"blue.tr.on_cooldown","fallback":" is on cooldown","color":"gray"}]
