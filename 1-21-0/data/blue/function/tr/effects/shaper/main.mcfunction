...
execute unless entity @s[tag=blue.tr.mats.powder] run return run execute positioned ^ ^0.5 ^1.5 as @e[distance=..16,tag=!blue.tr.shaper.checked,predicate=blue:shared/on_ground,type=item] if items entity @s contents tnt[count=1] at @s run function blue:tr/effects/shaper/place
