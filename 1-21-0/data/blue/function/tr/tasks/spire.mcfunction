advancement revoke @s only blue:tr/tasks/spire req
execute if score #.tr.spire_completed blue.config matches 2 unless entity @s[tag=blue.tr.spire] run return fail
...
execute unless score #.tr.random_task_defender blue.config matches 0 if entity @s[tag=!blue.tr.rtd_cooldown,predicate=blue:r/1c] run function blue:tr/tasks/rtd/spire