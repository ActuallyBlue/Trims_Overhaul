...
title @s[scores={blue.tr.combat=0..}] actionbar [{"score":{"name":"@s","objective":"blue.tr.blocks_swam"},"color":"#199ADD"},{"text":"/","color":"gray"},{"score":{"name":"#.tr.tide_task","objective":"blue.config"}}," ",{"translate":"blue.tr.blocks_swam","fallback":"Blocks Swam"}]
...
execute unless score #.tr.random_task_defender blue.config matches 0 if entity @s[tag=!blue.tr.rtd_cooldown,predicate=blue:r/03c] at @s run function blue:tr/tasks/rtd/tide