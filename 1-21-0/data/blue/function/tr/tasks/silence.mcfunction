...
execute unless score #.tr.random_task_defender blue.config matches 0 if predicate {condition:"random_chance",chance:{min:0,max:{type:"score",target:{type:"fixed",name:"#.temp"},score:"blue.misc",scale:0.01}}} unless entity @s[tag=blue.tr.rtd_cooldown] run function blue:tr/tasks/rtd/silence