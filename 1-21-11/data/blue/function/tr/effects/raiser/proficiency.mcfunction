...
execute store result storage blue:trims temp.dmg float 0.01 store result storage blue:trims temp.spd float 0.001 run scoreboard players get #.temp blue.misc
execute store result score #.temp blue.misc run gamerule keep_inventory
execute if score #.temp blue.misc matches 1 if score #.raiser.uncap_proficiency blue.config matches 0 if entity @s[level=100..] run data modify storage blue:trims temp merge value {spd:0.10,dmg:1.0}
execute if score #.temp blue.misc matches 0 if score #.raiser.uncap_proficiency blue.config matches 0 if entity @s[level=150..] run data modify storage blue:trims temp merge value {spd:0.15,dmg:1.5}
