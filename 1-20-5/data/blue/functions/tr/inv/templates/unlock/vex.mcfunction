...
execute if entity @s[tag=blue.tr.vex] run return fail
...
execute if score #.tr.objective_type blue.config matches 1 run tellraw @s [{"storage":"blue:trims","nbt":"unlock_text","interpret":true,"italic":true},{"translate":"blue.tr.vex_quest","fallback":"Strike down the one who mimics the Vex","color":"gray"}]
execute if score #.tr.objective_type blue.config matches 2 run tellraw @s [{"storage":"blue:trims","nbt":"unlock_text","interpret":true,"italic":true},{"translate":"blue.tr.vex_task","fallback":"Relentless Killer","color":"gray"}]