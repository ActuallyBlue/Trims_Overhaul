...
execute on origin if entity @s[tag=blue.tr.mats.shard] run scoreboard players set #.temp blue.misc -633
...
execute if score #.silence.vortex_cooldown blue.misc matches 1 on origin run title @s actionbar [{"translate":"blue.tr.vortex_name","fallback":"Vortex","color":"#3842Cf"},{"translate":"blue.tr.on_cooldown","fallback":" is on cooldown","color":"gray"}]
