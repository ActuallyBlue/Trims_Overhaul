...
execute if score #.ward.ambient blue.misc matches 0 run particle dust_color_transition{from_color:[0f,0.2f,0.5f],scale:0.65f,to_color:[0f,0f,0f]} ~ ~0.7 ~ 0.25 0.3 0.25 0 1 normal @a[scores={blue.particles=1..}]
execute if score #.ward.ambient blue.misc matches 1 run particle dust_color_transition{from_color:[0f,0.2f,0.5f],scale:0.65f,to_color:[0f,0f,0f]} ~ ~0.7 ~ 0.25 0.3 0.25 0 1 normal @a[scores={blue.particles=1..}]
...
scoreboard players operation @s blue.tr.damage_blocked /= #3 blue.misc
execute unless score #.20 blue.misc matches 9 run return fail
execute if biome ~ ~ ~ deep_dark run return run attribute @s generic.armor_toughness modifier add b163102f-0-5-0-1 "blue:tr.trim" 0.1 add_multiplied_total
