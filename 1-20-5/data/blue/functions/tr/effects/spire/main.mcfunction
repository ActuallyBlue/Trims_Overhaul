...
execute if score #.60 blue.misc matches 50 unless score #.spire.reversal_charge blue.misc matches ..-60 if score #.spire.no_gravity blue.config matches 1..2 run function blue:tr/effects/spire/check_setting
...
execute if score #.spire.reversal_charge blue.misc matches ..-1 run function blue:tr/effects/spire/emp/negative_charge
execute if entity @s[x_rotation=-90..-75] unless block ~ ~-1 ~ #replaceable unless score #.spire.reversal_charge blue.misc matches ..-1 rotated 0 0 run function blue:tr/effects/spire/emp/reversal/charge
execute if score #.spire.reversal_charge blue.misc matches 0.. if data entity @s {OnGround:0b} run function blue:tr/effects/spire/emp/reversal/release
execute if score #.spire.reversal_charge blue.misc matches 1.. run function blue:tr/effects/spire/emp/reversal/timer
execute as @e[tag=!blue.tr.checked,distance=..6,type=item] if items entity @s contents dragon_breath[count=1] run function blue:tr/effects/spire/emp/throw
