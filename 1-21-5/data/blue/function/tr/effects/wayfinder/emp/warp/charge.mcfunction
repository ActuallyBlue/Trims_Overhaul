...
execute store success score #.compass blue.misc run data modify storage blue:data trims.compass set from entity b163102f-0-0-0-100000000 equipment.mainhand
execute if score #.warp_charge blue.misc matches 1.. if score #.compass blue.misc matches 1 run return run scoreboard players reset #.warp_charging blue.misc
execute unless data storage blue:data trims.compass{id:"minecraft:compass",components:{"minecraft:lodestone_tracker":{target:{}}}} unless data storage blue:data trims.compass{id:"minecraft:recovery_compass"} run return fail
execute unless score #.wayfinder.warp_type blue.config matches 1..2 if data storage blue:data trims.compass{id:"minecraft:compass",components:{"minecraft:lodestone_tracker":{target:{}}}} run return run title @s actionbar [{"translate":"item.minecraft.lodestone_compass"},"-",{"translate":"blue.tr.warp_name","fallback":"Warp"},{"translate":"blue.tr.is_disabled","fallback":" is disabled on this server","color":"gray"}]
execute unless score #.wayfinder.warp_type blue.config matches 1 if data storage blue:data trims.compass{id:"minecraft:recovery_compass"} unless score #.wayfinder.warp_type blue.config matches 3 run return run title @s actionbar [{"translate":"item.minecraft.recovery_compass"},"-",{"translate":"blue.tr.warp_name","fallback":"Warp"},{"translate":"blue.tr.is_disabled","fallback":" is disabled on this server","color":"gray"}]
execute if data storage blue:data trims.compass.components."minecraft:custom_data".smithed.ignore run return run title @s actionbar {"translate":"blue.tr.important_compass","fallback":"This compass appears to be important to another datapack."}
...
execute if data storage blue:data trims.compass{id:"minecraft:compass"} if score #.warp_charge blue.misc matches 205.. run function blue:tr/effects/wayfinder/emp/warp/teleport
...
execute unless data storage blue:data trims.compass{id:"minecraft:compass"} if score #.warp_charge blue.misc matches 235.. run function blue:tr/effects/wayfinder/emp/warp/teleport
execute unless data storage blue:data trims.compass{id:"minecraft:compass"} at b163102f-0-3-0-1 run particle nautilus ~ ~1.5 ~ 0.3 0.45 0.3 1.4 2
