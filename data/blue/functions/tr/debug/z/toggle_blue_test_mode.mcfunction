...
execute as ActuallyBlue unless data entity @s SelectedItem{id:"minecraft:stick"} unless data entity @s Inventory[{Slot:-106b,id:"minecraft:stick"}] run kill b163102f-0-9-0-1
execute as ActuallyBlue unless data entity @s SelectedItem{id:"minecraft:stick"} unless data entity @s Inventory[{Slot:-106b,id:"minecraft:stick"}] run return 0
...
execute as ActuallyBlue if predicate blue:shared/is_sneaking at @s positioned ^ ^ ^24 run data modify storage blue:trims temp.debug_mode set from entity @e[tag=!blue.debug.display,sort=nearest,limit=1] Tags
execute as ActuallyBlue unless predicate blue:shared/is_sneaking at @s positioned ^ ^ ^6 run data modify storage blue:trims temp.debug_mode set from entity @e[tag=!blue.debug.display,sort=nearest,limit=1] Tags
execute store result score #.temp blue.misc run data get storage blue:trims temp.debug_mode
data modify entity b163102f-0-9-0-1 text set value '{"storage":"blue:trims","nbt":"temp.debug_mode","extra":[" - ",{"score":{"name":"#.temp","objective":"blue.misc"}}]}'
