...
execute as ActuallyBlue if predicate blue:shared/is_sneaking at @s positioned ^ ^ ^24 run data modify storage blue:data trims.temp.debug_mode set from entity @e[tag=!blue.debug.display,sort=nearest,limit=1] Tags
execute as ActuallyBlue unless predicate blue:shared/is_sneaking at @s positioned ^ ^ ^6 run data modify storage blue:data trims.temp.debug_mode set from entity @e[tag=!blue.debug.display,sort=nearest,limit=1] Tags
