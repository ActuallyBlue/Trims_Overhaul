...
execute if predicate blue:shared/is_sneaking run effect give @s slow_falling 1 0 true
execute if predicate blue:tr/speed_2 run function blue:tr/effects/wayfinder/fast
execute if predicate blue:tr/holding_rod as @e[distance=1.25..32,tag=!blue.tr.checked,type=fishing_bobber] at @s run function blue:tr/effects/wayfinder/grapple
...
scoreboard players reset #.wayfinder.warp_charging blue.misc
execute unless score #.wayfinder.warp_cooldown blue.misc matches 1 if entity @s[scores={blue.tr.combat=0..},predicate=blue:tr/hold_compass] run function blue:tr/effects/wayfinder/emp/warp/charge
execute if score #.wayfinder.warp_charge blue.misc matches 1.. unless score #.wayfinder.warp_charging blue.misc matches 1 run function blue:tr/effects/wayfinder/emp/warp/exit
...
execute if dimension overworld if score #.level blue.tr.structures matches 32.. if score #.wayfinder.void_return blue.config matches 1 unless predicate blue:shared/void if entity @s[y=0,dy=-10000] run function blue:tr/effects/wayfinder/emp/return
execute unless entity @s[predicate=blue:shared/is_sneaking,predicate=blue:tr/explorer] run return 0
