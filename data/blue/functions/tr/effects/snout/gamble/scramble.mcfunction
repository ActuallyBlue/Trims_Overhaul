...
execute at @a[distance=..16,gamemode=!spectator,gamemode=!creative] run summon marker ~ ~ ~ {Tags:["blue.tr.gamble.scramble"]}
...
execute as @a[distance=..16,gamemode=!spectator,gamemode=!creative] at @e[tag=blue.tr.gamble.scramble,distance=..40,sort=random,limit=1,type=marker] run function blue:tr/effects/snout/gamble/scrambled
