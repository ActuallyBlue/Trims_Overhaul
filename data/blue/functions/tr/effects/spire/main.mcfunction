...
execute if score #.sneaking blue.misc matches 1 if block ~ ~-0.5 ~ #replaceable run effect give @s[advancements={blue:tr/tags={not_sneaking=false}}] slow_falling 1 0 true
execute if score #.sneaking blue.misc matches 1 if block ~ ~-0.5 ~ #replaceable unless score #.spire.guidance_cooldown blue.misc matches 1 if entity @s[advancements={blue:tr/tags={not_sneaking=true}}] run function blue:tr/effects/spire/guidance
