...
execute if score #.temp blue.misc matches -1 positioned ~-0.5 ~-0.5 ~-0.5 if entity @e[type=!#blue:shared/no_effects,tag=!blue.tr.spire,dx=0,limit=1] run scoreboard players set #.temp blue.misc 1
execute if score #.temp blue.misc matches ..0 run return 0
playsound entity.splash_potion.break player @a ~ ~ ~ 0.8 0.75
kill
execute positioned ~-3 ~-1 ~-3 if entity @e[type=area_effect_cloud,tag=blue.tr.blast-o-breath,dx=5,dy=2,dz=5,limit=1] run return 0
summon area_effect_cloud ~ ~ ~ {Rotation:[0,180],Tags:["blue.tr.blast-o-breath"],"Potion":"harming",Particle:"block air",Duration:15,Radius:0.5,RadiusPerTick:0.15}
summon area_effect_cloud ~ ~ ~ {Rotation:[0,180],Particle:"dragon_breath",Duration:15,Radius:1.5,RadiusPerTick:0.1}
