...
kill
execute positioned ~-3 ~-1 ~-3 if entity @e[tag=blue.tr.blast-o-breath,dx=5,dy=2,dz=5,limit=1,type=area_effect_cloud] run return 0
summon area_effect_cloud ~ ~ ~ {Rotation:[0,180],Tags:["blue.tr.blast-o-breath"],"Potion":"harming",Particle:"block air",Duration:16,Radius:0.5,RadiusPerTick:0.15,ReapplicationDelay:0,WaitTime:0s}
data modify entity @e[tag=blue.tr.blast-o-breath,type=area_effect_cloud,distance=..0.1,limit=1] Owner set from entity @a[scores={blue.tr.current_trim=11},limit=1] UUID
summon area_effect_cloud ~ ~ ~ {Rotation:[0,180],Particle:"dragon_breath",Duration:15,Radius:1.5,RadiusPerTick:0.1,WaitTime:0s}
