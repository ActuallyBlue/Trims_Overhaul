data modify storage blue:trims temp.effects set from entity @s ActiveEffects[{Id:32}]
...
data remove storage blue:trims temp.effects
execute if score #.sentry.charge blue.misc matches -140 run scoreboard players remove #.temp blue.misc 18000
...
summon area_effect_cloud ~ ~1 ~ {UUID:[I;-1318907857,16,0,1],Tags:["blue.tr.sentry.refresh_cloud"],Duration:99,DurationOnUse:-99,Radius:3,WaitTime:0,Particle:"block air"}
execute store result storage blue:trims merge.aec.Effects[0].Duration int 1 run scoreboard players get #.temp blue.misc
execute store result storage blue:trims merge.aec.Effects[0].Amplifier int 1 run scoreboard players get #.temp0 blue.misc
