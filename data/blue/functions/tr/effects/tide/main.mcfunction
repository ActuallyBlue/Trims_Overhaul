...
execute unless entity @s[tag=blue.tr.mats.heart] run return 0
  ...
  execute if predicate blue:tr/in_monument run scoreboard players remove #.tide.recharge blue.misc 30
  execute unless score #.tide.stack_cooldown blue.misc matches 1 unless score #.tide.stacks blue.misc matches 10.. run scoreboard players add #.tide.timer blue.misc 1
