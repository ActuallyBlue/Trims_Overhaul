...
execute unless score #.rib.skull_cooldown blue.misc matches 1 if predicate blue:shared/is_sneaking anchored eyes run function blue:tr/effects/rib/skull/detect
execute if score #.rib.was_sneaking blue.misc matches 1 unless predicate blue:shared/is_sneaking run function blue:tr/effects/rib/skull/reset
execute as @e[type=fireball,distance=..20,tag=!blue.tr.rib_deflected] at @s run function blue:tr/effects/rib/deflect/check
...
execute unless score #.20 blue.misc matches 3 run return 0
effect give @s fire_resistance 2 0 true
effect clear @s wither
execute unless predicate blue:shared/has_vehicle run return 0
