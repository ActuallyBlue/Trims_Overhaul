...
execute positioned ~-0.5 ~-0.5 ~-0.5 as @e[type=!#blue:tr/peaceful,tag=!blue.tr.coast,dx=0,sort=nearest,limit=1] positioned ~0.5 ~0.5 ~0.5 run function blue:tr/effects/coast/fire/proj/tropical/hit
execute if score #.temp blue.misc matches -99 run kill
execute if block ~ ~ ~ #blue:trims/passable if block ^ ^ ^0.3 #blue:trims/passable if block ^ ^ ^0.6 #blue:trims/passable run return 0
