...
execute if score #.shaper.throw_cooldown blue.config matches -1 on origin run title @s actionbar [{"translate":"blue.tr.not-so_controlled_demolition_name","fallback":"Not-so Controlled Demolition","color":"white"},{"translate":"blue.tr.is_disabled","fallback":" is disabled on this server","color":"gray"}]
...
execute if score #.shaper.throw_cooldown blue.misc matches 1.. run function blue:tr/item/quick_return
