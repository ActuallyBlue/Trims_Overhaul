...
execute if entity @e[distance=..8,limit=1,nbt={TNTFuse:0},type=tnt_minecart] run function blue:tr/effects/shaper/invul
execute positioned ^ ^0.5 ^1.5 as @e[distance=..6,tag=!blue.tr.checked,type=item] run function blue:tr/effects/shaper/item
execute unless entity @s[tag=blue.tr.mats.powder] positioned ^ ^0.5 ^1.5 as @e[distance=..16,tag=!blue.tr.shaper_checked,nbt={OnGround:1b,Item:{id:"minecraft:tnt",Count:1b}},type=item] at @s run function blue:tr/effects/shaper/place
execute unless entity @s[tag=blue.tr.mats.powder] run return 0
