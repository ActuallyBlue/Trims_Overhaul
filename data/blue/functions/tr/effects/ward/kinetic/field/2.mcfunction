...
execute as @a[distance=..7.25,advancements={blue:tr/trust={ward=true}},gamemode=!spectator] positioned ~-7.25 ~-1.75 ~-7.25 run tag @s[dx=27,dy=4.5,dz=27] add blue.tr.ward.in_field
effect give @a[tag=blue.tr.ward.in_field,predicate=!blue:shared/has_regen] regeneration 3 0 true
effect give @a[tag=blue.tr.ward.in_field,scores={blue.food=..10}] saturation 1 0 true
execute at @a[tag=blue.tr.ward.in_field] run particle electric_spark ~ ~0.1 ~ 0.2 0 0.2 0.1 2
