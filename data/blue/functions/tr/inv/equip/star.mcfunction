execute if score #.tr.wayfinder_toggle blue.config matches 2 run tellraw @s {"translate":"blue.tr.empowerment_disabled","fallback":"- %s empowerment is disabled on this server.","color":"gray","with":[{"translate":"Wayfinder","extra":["'s"],"color":"#F1FFC8"}]}
execute if score #.tr.wayfinder_toggle blue.config matches 2 run return 0
