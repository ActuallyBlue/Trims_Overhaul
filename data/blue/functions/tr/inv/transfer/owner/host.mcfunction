...
execute if entity @a[tag=blue.tr.transfer_failed,advancements={blue:tr/blacklist={host=true}}] run tellraw @s [{"text":"- ","color":"gray"},{"selector":"@a[tag=blue.tr.transfer_target]"}," ",{"translate":"blue.tr.transfer_blacklist","fallback":"has been blacklisted from getting"}," ",{"translate":"Host","color":"#A34646"}]
execute if entity @a[tag=blue.tr.transfer_failed,advancements={blue:tr/blacklist={host=true}}] run return 0
