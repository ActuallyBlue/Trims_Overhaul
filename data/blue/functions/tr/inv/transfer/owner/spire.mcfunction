...
execute if entity @a[tag=blue.tr.transfer_failed,advancements={blue:tr/blacklist={spire=true}}] run tellraw @s [{"text":"- ","color":"gray"},{"selector":"@a[tag=blue.tr.transfer_target]"}," ",{"translate":"blue.tr.transfer_blacklist","fallback":"has been blacklisted from getting"}," ",{"translate":"Spire","color":"#BE20C7"}]
execute if entity @a[tag=blue.tr.transfer_failed,advancements={blue:tr/blacklist={spire=true}}] run return 0
