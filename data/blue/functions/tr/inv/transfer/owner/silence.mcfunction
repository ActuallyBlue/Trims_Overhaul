...
execute if entity @a[tag=blue.tr.transfer_failed,advancements={blue:tr/blacklist={silence=true}}] run tellraw @s [{"text":"- ","color":"gray"},{"selector":"@a[tag=blue.tr.transfer_target]"}," ",{"translate":"blue.tr.transfer_blacklist","fallback":"has been blacklisted from getting"}," ",{"translate":"Silence","color":"#3842Cf"}]
execute if entity @a[tag=blue.tr.transfer_failed,advancements={blue:tr/blacklist={silence=true}}] run return 0
