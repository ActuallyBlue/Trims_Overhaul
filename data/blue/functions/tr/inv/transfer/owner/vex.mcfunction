...
execute if entity @a[tag=blue.tr.transfer_target,advancements={blue:tr/blacklist={vex=true}}] run tellraw @s [{"text":"- ","color":"dark_gray"},{"selector":"@a[tag=blue.tr.transfer_target]"}," ",{"translate":"blue.tr.transfer_blacklist","fallback":"has been blacklisted from getting","color":"gray"}," ",{"translate":"Vex","color":"#CFC6A5"}]
