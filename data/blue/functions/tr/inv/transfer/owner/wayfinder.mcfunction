...
execute if entity @a[tag=blue.tr.transfer_failed,advancements={blue:tr/blacklist={wayfinder=true}}] run tellraw @s [{"text":"- ","color":"gray"},{"selector":"@a[tag=blue.tr.transfer_target]"}," ",{"translate":"blue.tr.transfer_blacklist","fallback":"has been blacklisted from getting"}," ",{"translate":"Wayfinder","color":"#F1FFC8"}]
execute if entity @a[tag=blue.tr.transfer_failed,advancements={blue:tr/blacklist={wayfinder=true}}] run return 0
