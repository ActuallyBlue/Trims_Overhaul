...
execute if entity @a[tag=blue.tr.transfer_failed,advancements={blue:tr/blacklist={coast=true}}] run tellraw @s [{"text":"- ","color":"gray"},{"selector":"@a[tag=blue.tr.transfer_target]"}," ",{"translate":"blue.tr.transfer_blacklist","fallback":"has been blacklisted from getting"}," ",{"translate":"Coast","color":"#C1C888"}]
execute if entity @a[tag=blue.tr.transfer_failed,advancements={blue:tr/blacklist={coast=true}}] run return 0
