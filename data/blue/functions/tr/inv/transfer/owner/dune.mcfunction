...
execute if entity @a[tag=blue.tr.transfer_failed,advancements={blue:tr/blacklist={dune=true}}] run tellraw @s [{"text":"- ","color":"gray"},{"selector":"@a[tag=blue.tr.transfer_target]"}," ",{"translate":"blue.tr.transfer_blacklist","fallback":"has been blacklisted from getting"}," ",{"translate":"Dune","color":"#E7C547"}]
execute if entity @a[tag=blue.tr.transfer_failed,advancements={blue:tr/blacklist={dune=true}}] run return 0
