...
execute if entity @a[tag=blue.tr.transfer_failed,advancements={blue:tr/blacklist={rib=true}}] run tellraw @s [{"text":"- ","color":"gray"},{"selector":"@a[tag=blue.tr.transfer_target]"}," ",{"translate":"blue.tr.transfer_blacklist","fallback":"has been blacklisted from getting"}," ",{"translate":"Rib","color":"#E6484B"}]
execute if entity @a[tag=blue.tr.transfer_failed,advancements={blue:tr/blacklist={rib=true}}] run return 0
