...
execute if entity @a[tag=blue.tr.transfer_target,advancements={blue:tr/blacklist={eye=true}}] run tellraw @s [{"text":"- ","color":"dark_gray"},{"selector":"@a[tag=blue.tr.transfer_target]"}," ",{"translate":"blue.tr.transfer_blacklist","fallback":"has been blacklisted from getting","color":"gray"}," ",{"translate":"Eye","color":"#1F8A46"}]
execute if entity @a[tag=blue.tr.transfer_target,advancements={blue:tr/blacklist={eye=true}}] run return 0
