...
execute if entity @a[tag=blue.tr.transfer_target,advancements={blue:tr/blacklist={raiser=true}}] run tellraw @s [{"text":"- ","color":"gray"},{"selector":"@a[tag=blue.tr.transfer_target]"}," ",{"translate":"blue.tr.transfer_blacklist","fallback":"has been blacklisted from getting"}," ",{"translate":"Raiser","color":"#95B623"}]
