...
execute if score #.tr.objective_type blue.config matches 3 if score #.tr.limit_owned_trims blue.config matches 1.. if entity @s[advancements={blue:tr/tags={trim_limit=false}}] if score #.tr.silence_completed blue.config matches 0 if entity @s[advancements={blue:tr/blacklist={silence=false}}] run function blue:tr/completion/silence
execute if score #.tr.template_clearing blue.config matches 0 run return 0
execute store result score #.clear_temp blue.misc run clear @s silence_armor_trim_smithing_template{blue:{trim:1b}}
...
execute at @a[tag=blue.tr.silence,limit=1] run loot spawn ~ ~0.5 ~ loot blue:tr/silence
