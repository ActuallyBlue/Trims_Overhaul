execute if predicate blue:tr/alldura run function blue:tr/inv/mats/dura/any_low
execute if data storage blue:data trims.inventory[].tag.Trim{material:"dragon_egg"} run function blue:tr/inv/mats/has_egg
execute store result score #.temp blue.misc run data get storage blue:data trims.armor
execute if score #.temp blue.misc matches 4 store success score #.valid_slots blue.misc run data modify storage blue:data trims.inventory[].tag.Trim.material set from storage blue:data trims.inventory[-1].tag.Trim.material
