...
scoreboard players operation #.link blue.id = @e[tag=blue.tr.quest.vex_passenger,predicate=!blue:shared/has_vehicle,sort=nearest,limit=1,type=marker] blue.id
execute unless predicate blue:shared/id_link run return 0
kill @e[tag=blue.tr.quest.vex_passenger,predicate=blue:shared/id_link,type=marker]
