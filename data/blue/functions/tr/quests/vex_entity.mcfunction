...
execute on passengers run scoreboard players operation #.link blue.id = @s blue.id
...
execute if score #.temp blue.misc matches 0 run data merge entity @s {ArmorItems:[{},{},{},{}],DeathTime:20s,Health:0f}
...
kill @e[distance=..5,tag=!smithed.entity,tag=!smithed.strict,type=iron_golem]
...
execute facing entity @a[predicate=blue:shared/id_link,distance=..12,limit=1] eyes positioned 0.0 0 0.0 rotated ~ -16 positioned ^ ^ ^1.7 summon marker run function blue:shared/shootfacing
