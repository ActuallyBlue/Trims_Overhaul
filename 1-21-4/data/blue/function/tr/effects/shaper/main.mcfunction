...
execute if entity @e[type=tnt_minecart,distance=..8,limit=1,nbt={fuse:1}] run function blue:tr/effects/shaper/invul
execute positioned ^ ^0.5 ^1.5 as @e[type=item,distance=..6,tag=!blue.tr.checked] run function blue:tr/effects/shaper/item
execute unless entity @s[tag=blue.tr.mats.powder] run return run execute positioned ^ ^0.5 ^1.5 as @e[type=item,distance=..12,tag=!blue.tr.shaper_checked,predicate=blue:shared/on_ground] if items entity @s contents tnt[count=1] at @s run function blue:tr/effects/shaper/place
...
execute if score #.difficulty blue.misc matches ..1 run return run tag @s[predicate=blue:tr/low_fall] add blue.tr.shaper_falling
execute if score #.difficulty blue.misc matches 2 run tag @s[predicate=blue:tr/mid_fall] add blue.tr.shaper_falling